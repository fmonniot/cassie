...
trait ByNameScalarRowDecoder[A] {
  def decode(row: GettableByNameData, name: String): Either[CassieError, A]
}

object ByNameScalarRowDecoder {
  def apply[A](implicit dec: ByNameScalarRowDecoder[A]): ByNameScalarRowDecoder[A] = dec

  def pure[A](func: (GettableByNameData, String) => Either[CassieError, A]): ByNameScalarRowDecoder[A] =
